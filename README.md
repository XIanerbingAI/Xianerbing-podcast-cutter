...
# 3) 配置(可选,不配也能跑 —— 纯离线规则)
cp .env.example .env
#   - WHISPER_MODEL=medium (无 GPU 建议 medium,有 GPU 用 large-v3)
#   - LLM_* (可选,填了启用大模型复核)
...
首次转写会下载 Whisper 模型(`medium`≈1.5GB,`large-v3`≈3GB),缓存到 `models/`。
...
**Q: 首次启动很慢?**
A: 在下载 Whisper 模型(1.5–3GB),之后缓存复用。

**Q: 没有 GPU 能用吗?**
A: 能。`medium` 模型在 CPU 上约 1× 实时速度(10 分钟音频约 10 分钟处理),可接受。
